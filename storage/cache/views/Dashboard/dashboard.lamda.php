                ...
                <ul class="space-y-3 max-h-96 overflow-y-auto">
                    <?php foreach ($news as $item): ?>
                        <li class="p-3 bg-gray-50 border-l-4 border-blue-500 rounded">
                            <p class="font-medium text-gray-800"><?=  htmlspecialchars($item['title'], ENT_QUOTES, 'UTF-8') ?></p>
                            <p class="text-sm text-gray-600"><?=  htmlspecialchars($item['content'], ENT_QUOTES, 'UTF-8') ?></p>
                            <div class="mt-3 flex gap-2">
                                <a href="/news/<?=  htmlspecialchars($item['slug'], ENT_QUOTES, 'UTF-8') ?>" class="px-3 py-1 bg-green-500 hover:bg-green-600 text-white text-sm rounded transition">Lihat</a>
                                <a href="/dashboard/update/<?=  htmlspecialchars($item['id'], ENT_QUOTES, 'UTF-8') ?>" class="px-3 py-1 bg-yellow-500 hover:bg-yellow-600 text-white text-sm rounded transition">Update</a>
                                <a href="/dashboard/delete/<?=  htmlspecialchars($item['id'], ENT_QUOTES, 'UTF-8') ?>" class="px-3 py-1 bg-red-500 hover:bg-red-600 text-white text-sm rounded transition">Delete</a>
                            </div>
                        </li>
                    <?php endforeach; ?>
                </ul>
