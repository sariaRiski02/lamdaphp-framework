                    ...
                    <div class="space-y-3">
                        <?php foreach ($categories as $category): ?>
                            <div class="flex justify-between items-center p-4 bg-gray-100 rounded-lg">
                                <span class="font-semibold text-gray-800"><?=  htmlspecialchars($category['name'], ENT_QUOTES, 'UTF-8') ?></span>
                                <div class="flex gap-2">
                                    <a href="/dashboard/category/update/<?=  htmlspecialchars($category['slug'], ENT_QUOTES, 'UTF-8') ?>" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded text-sm">Edit</a>
                                    <form action="/dashboard/category/delete/<?=  htmlspecialchars($category['slug'], ENT_QUOTES, 'UTF-8') ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this category?');">
