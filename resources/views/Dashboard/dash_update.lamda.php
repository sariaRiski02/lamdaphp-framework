    ...
    <div class="min-h-screen py-12 px-4">
        <div class="max-w-2xl mx-auto">
            ...
            <div class="bg-white rounded-lg shadow-md p-6 mb-8">
                ...
                <form action="/dashboard/update/{{$news['id']}}" method="POST" class="space-y-4">
                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700 mb-1">Judul:</label>
                        <input type="text" id="title" name="title" value="{{$news['title']}}" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label for="content" class="block text-sm font-medium text-gray-700 mb-1">Konten:</label>
                        <textarea id="content" name="content" rows="4" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">{{$news['content']}}</textarea>
                    </div>
                    <div>
