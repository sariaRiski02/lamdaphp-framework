    ...
    <div class="max-w-4xl mx-auto px-4 py-12">
        ...
        <article class="bg-white rounded-2xl shadow-lg overflow-hidden">
            ...
            <div class="relative h-96 bg-gray-200 overflow-hidden">
                ...
                <div class="absolute top-4 right-4 bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-2 rounded-full text-sm font-semibold">
                    <i class="fas fa-star mr-1"></i> {{ $category['name'] ?? 'default' }}
                </div>
            </div>
            ...
            <div class="p-8 md:p-12">
                ...
                <div class="flex flex-wrap gap-3 mb-8 pt-8 border-t border-gray-200">
                    ...
                    <a href="#" class="inline-block bg-purple-100 text-purple-700 px-4 py-2 rounded-full text-sm font-medium hover:bg-purple-200 transition-colors">
                        <i class="fas fa-tag mr-2"></i>{{ $category['name'] ?? 'default' }}
                    </a>
                </div>
            ...
            <div class="bg-gray-50 px-8 md:px-12 py-6 border-t border-gray-200 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="font-semibold text-gray-900">{{ $user['name'] ?? 'default' }}</p>
        ...
        <div class="mt-16">
            ...
            @foreach($newsRelated as $item)
                <!-- Related News Card -->
                <a href="/detail/{{$item['id']}}" class="group bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    ...
                    <div class="p-4">
                        <h3 class="text-sm font-bold text-gray-900 group-hover:text-purple-600 transition-colors">{{ $item['title'] }}</h3>
                        <p class="text-xs text-gray-500 mt-2">{{ $item['created_at'] }}</p>
                    </div>
                </a>
            @endforeach
